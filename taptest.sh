...
DEV=tap0
# Set tunnel's IP:
sudo ip addr add 192.168.0.1/24 dev $DEV # ip addr will now show our tun0 interface after this command.

sudo ip link set up dev $DEV # This links our tun0 with another interface causing it to read data.
ping -I $DEV 192.168.0.2 & # Start pinging on the tun0 device.
